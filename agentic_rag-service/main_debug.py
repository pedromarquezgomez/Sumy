    ...
    try:
        ...
        if rag_engine.openai_client:
            try:
                ...
                logger.info(f"Intentando llamada a OpenAI con modelo: {OPENAI_MODEL}")
                ...
                logger.info("Llamada a OpenAI exitosa")
            except Exception as openai_error:
                logger.error(f"Error específico de OpenAI: {type(openai_error).__name__}: {str(openai_error)}")
                llm_answer = f"Error de OpenAI: {type(openai_error).__name__}: {str(openai_error)}"
        else:
            llm_answer = f"Cliente OpenAI no inicializado. Basado en {len(sources)} fuentes encontradas para: '{query_data.query}'"
...
async def debug_connectivity():
    """Endpoint de diagnóstico de conectividad"""
    import requests
    import socket
    ...
    # Test 2: Conectividad HTTP básica
    try:
        response = requests.get("https://api.openai.com/v1/models", timeout=10)
        results["http_connectivity"] = {"status": "success", "status_code": response.status_code}
    except Exception as e:
        results["http_connectivity"] = {"status": "error", "error": str(e)}
    
    # Test 3: Verificar variable de entorno
    openai_key = os.getenv("OPENAI_API_KEY")
    if openai_key:
        results["api_key"] = {"status": "present", "prefix": openai_key[:10] + "..."}
    else:
        results["api_key"] = {"status": "missing"}
    
    # Test 4: Test con OpenAI client
    try:
        if rag_engine.openai_client:
            test_response = rag_engine.openai_client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[{"role": "user", "content": "test"}],
                max_tokens=5,
                timeout=10
            )
            results["openai_client"] = {"status": "success", "response_id": test_response.id}
        else:
            results["openai_client"] = {"status": "client_not_initialized"}
    except Exception as e:
        results["openai_client"] = {"status": "error", "error": str(e), "error_type": type(e).__name__}
    
    # Test 5: Test directo con requests a OpenAI
    try:
        openai_key = os.getenv("OPENAI_API_KEY")
        if openai_key:
            headers = {
            ...
            if response.status_code == 200:
                results["direct_openai"] = {"status": "success", "status_code": response.status_code}
            else:
                results["direct_openai"] = {"status": "error", "status_code": response.status_code, "error": response.text[:200]}
        else:
            results["direct_openai"] = {"status": "no_api_key"}
    except Exception as e:
        results["direct_openai"] = {"status": "error", "error": str(e), "error_type": type(e).__name__}
