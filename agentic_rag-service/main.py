#!/usr/bin/env python3
"""
Servidor RAG MCP Agéntico - VERSIÓN CORREGIDA
...
async def query_rag_mcp(query_data: QueryRequest):
    ...
    logger.info(f"🔍 CONSULTA RECIBIDA: '{query_data.query}'")
    ...
    try:
        ...
        logger.info(f"📊 Total documentos en BD: {total_docs}")
        
        if total_docs == 0:
            logger.warning("⚠️ No hay documentos en la base de datos")
            return {
                "answer": "No hay documentos cargados en la base de conocimientos.",
                "sources": [],
                "context_used": {"query": query_data.query, "total_docs": 0}
            }

        # ✅ CORRECCIÓN CRÍTICA: Usar pipeline agéntico completo
        logger.info("🤖 Iniciando proceso RAG agéntico...")
        response = await rag_engine.agentic_rag_query(
        ...
        end_total = time.time()
        logger.info(f"⏱️ Consulta completada en {end_total - start_total:.2f}s")
        logger.info(f"📊 Fuentes encontradas: {len(response.sources)}")
...
async def debug_status():
    """Diagnóstico rápido del sistema RAG agéntico"""
    try:
        if rag_engine.collection is None:
            await rag_engine.initialize()
            
        total_docs = rag_engine.collection.count() if rag_engine.collection else 0
        
        return {
            "status": "ok",
            "total_documents": total_docs,
            "agentic_pipeline": "enabled",
            "openai_available": rag_engine.openai_client is not None,
            "fix_applied": True,
            "timestamp": time.time()
        }
    except Exception as e:
